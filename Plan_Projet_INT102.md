...
| M7b | Document-editing tool *(core, after M7a)* | `Document`→`DocumentVersion` restructuring migration (all formats, composite ownership FKs, two checksums `source`/`text`, write-once per-version chunking); anchored-patch flow for TXT/MD (server-owned context + staleness pins, raw-equality unique anchors via `find_all_exact_anchors`, human-edited final text, diff review, token-fenced two-phase approval gate); `RemediationArtifact` + `supersedes_version_id` re-upload flow with corrective-action lineage for PDF/DOCX; `PENDING_INDEX | ACTIVE | SUPERSEDED | INDEX_FAILED | ABANDONED` activation protocol (CAS + crash-safe recovery) with snapshot-consistent current-version retrieval (PG-authoritative, `CorpusChangedError` on mid-search activation); remediation evaluation corpus + metrics |
