...
const STATUS_CLASSES: Record<SoaControlRow["status"], string> = {
  ...
  non_evalue: "bg-slate-100 text-slate-500",
};
...
export default function SoaPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {soa.isLoading && <p className="text-sm text-slate-500">Chargement…</p>}
      {soa.error && (
  ...
  return (
    <li className="rounded-xl border border-slate-200 bg-white p-4 text-sm">
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-medium text-slate-800">{c.control_id}</span>
        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_CLASSES[c.status]}`}>
          {STATUS_LABELS[c.status]}
        </span>
        <span
          className={`rounded-full px-2 py-0.5 text-xs font-medium ${
            c.applicable ? "bg-indigo-100 text-indigo-700" : "bg-slate-200 text-slate-600"
          }`}
        >
          {c.applicable ? "applicable" : "non applicable"}
        </span>
        {c.is_default && <span className="text-xs text-slate-400">(par défaut)</span>}
      ...
      {c.requirement_fr && <p className="mt-1 text-xs text-slate-500">{c.requirement_fr}</p>}
      {c.justification_fr && !editing && (
        <p className="mt-1 text-xs text-slate-600">Justification : {c.justification_fr}</p>
      )}
      ...
      {editing && (
        ...
        >
          ...
          <label className="block">
            <span className="text-xs text-slate-500">Justification (requise)</span>
            <textarea
              value={justification}
      ...
      {showHistory && (
        <div className="mt-3 rounded-lg bg-slate-50 p-3">
          <h3 className="text-xs font-semibold text-slate-600">Historique des décisions</h3>
